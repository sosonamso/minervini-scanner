...
if __name__=="__main__":
    ...
    if not res:
        import pandas as pd
        pd.DataFrame(columns=['date', 'ticker', 'cap', 'cur', 'pivot', 'cup_depth', 'handle_depth', 'cup_days', 'handle_days', 'vol_ratio', 'rs', 'score', 'grade']).to_csv("scanner_us_small_raw.csv",index=False,encoding="utf-8-sig")
        print(f"빈 CSV 저장: scanner_us_small_raw.csv")
