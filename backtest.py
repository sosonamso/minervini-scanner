...
def trend(df):
 if len(df)<150:return False  # 200→150으로 완화
 c=df["Close"]
 m5=c.rolling(50).mean();m15=c.rolling(150).mean();m20=c.rolling(200).mean()
 cur=float(c.iloc[-1])
 a=float(m5.iloc[-1]);b=float(m15.iloc[-1])
 m20_valid=m20.dropna()
 if len(m20_valid)<1:return False
 d=float(m20_valid.iloc[-1])
 d1=float(m20_valid.iloc[-21]) if len(m20_valid)>=21 else d
 if any(pd.isna([a,b,d])):return False
 lk=c.iloc[-252:]if len(c)>=252 else c
 # 조건 완화: 52주 조건 제거, MA 정배열만 체크
 return all([cur>b and cur>d,b>d,d>d1,a>b,cur>a])

def cup(df):
 cl=df["Close"].values.flatten();vl=df["Volume"].values.flatten();n=len(cl)
 ...
 if not(0.12<=cd<=0.55)or(bi-li)<25:return False,{}  # 조건 완화
 rc=c[bi:]
 if len(rc)<10:return False,{}
 ri=bi+int(np.argmax(rc));rh=c[ri]
 if rh<lh*0.85:return False,{}  # 완화
 hd=c[ri:];hl=len(hd)
 if not(3<=hl<=25):return False,{}  # 완화
 hl2=float(np.min(hd));hdd=(rh-hl2)/rh
 if not(0.03<=hdd<=0.20):return False,{}  # 완화
 if(hl2-bot)/(lh-bot)<0.50:return False,{}  # 완화
 cur=cl[-1]
 if not(rh*0.95<=cur<=rh*1.08):return False,{}  # 완화
...
if __name__=="__main__":
 ...
 send(f"스캔시작: {len(TICKERS)}개 (완화조건)")
 res=[];trend_pass=0
 for i,t in enumerate(TICKERS):
  if i%20==0:print(f"[{i}/{len(TICKERS)}] 트렌드:{trend_pass} 발견:{len(res)}")
  df=get_ohlcv(t,s,e)
 ...
 send(f"완료:{len(res)}건 (트렌드통과:{trend_pass})")
